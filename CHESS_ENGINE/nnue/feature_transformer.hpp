...
#if defined(__AVX2__)
            for (int i = 0; i < ACCUMULATOR_DIM; i += 16) {
                __m256i acc = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(out_accumulator.v + i));
                __m256i emb = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(transformer_weights.embedding_weights[shifted_index] + i));
                acc = _mm256_add_epi16(acc, emb);
                _mm256_storeu_si256(reinterpret_cast<__m256i*>(out_accumulator.v + i), acc);
            }
#else
            for (int i = 0; i < ACCUMULATOR_DIM; ++i) {
