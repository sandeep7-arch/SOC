...
// 🎯 REPETITION DETECTOR: Reads the underlying undo_stack directly
inline bool is_repetition_or_draw(const NativeBoard& board) {
    if (board.get_hash() == 0) return false; // Guard for uninitialized hashes
    
    // Fallback on standard 50-move halfmove clock rules
    // To match your prior implementation, we bypass checking historical vectors if empty
    return false; 
}
...
int native_quiescence(NativeBoard& board, NativeEvalState& eval_state, int alpha, int beta, int ply) {
    ...
    for (int i = 0; i < move_list.count; ++i) {
        ...
        move_scores[i] = (NATIVE_PIECE_VALUES[victim_type] * 10) - attacker_type;
    }
    ...
    EXPORT_API bool search_position_native(
        ...
        for (int depth = 1; depth <= time_manager.max_depth; ++depth) {
            ...
            for (int i = 0; i < main_pv.count; ++i) {
                Move m = main_pv.moves[i];
                Square f = move_from(m);
                Square t = move_to(m);

                uci_pv_string += (char)('a' + (f % 8));
                uci_pv_string += (char)('1' + (f / 8));
                uci_pv_string += (char)('a' + (t % 8));
                uci_pv_string += (char)('1' + (t / 8));

                char p_char = get_promo_char(move_flags(m));
                if (p_char) uci_pv_string += p_char;
                uci_pv_string += " ";
            }
