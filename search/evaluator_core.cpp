...
public:
    ...
    NativeEvaluator(size_t cache_size) : active_entries(0) {
        ...
        cache_table.resize(table_size, {0, 0, false, false});
    }
    ...
    int probe(uint64_t hash, bool turn, int* out_found) {
        ...
        if (entry.valid && entry.hash == hash && entry.turn == turn) {
            cache_hits++;
    ...
    void store(uint64_t hash, bool turn, int score) {
        ...
        cache_table[index] = {hash, score, turn, true};
    }

    void clear() {
        std::fill(cache_table.begin(), cache_table.end(), EvalEntry{0, 0, false, false});
        active_entries = 0;
        cache_hits = 0;
        cache_misses = 0;
    }
