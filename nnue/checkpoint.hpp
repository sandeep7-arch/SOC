...
inline const std::string CHECKPOINT_VERSION = "2.0"; // Upgraded structural layout version tracking

// Fully allocated container representing all trained dense layer memory blocks
struct ModelWeights {
    // Layer 1 (1024 -> 256)
...
public:
    ...
    static bool save_checkpoint(const std::string& filepath,
        ...
        // 3. Complete Deep Network Model Weights & Biases Blocks Serializations
    ...
    static bool load_checkpoint(const std::string& filepath, CheckpointState& out_state) {
        ...
        // 3. Unpack complete deep network parameter structures
    ...
    static std::string latest_checkpoint(const std::string& directory) {
        ...
        // 🎯 Priority Target Fix: Match against the absolute system master binary block (.ckpt)
        fs::path master_file = fs::path(directory) / "nnue_brain.ckpt";
        if (fs::exists(master_file) && fs::is_regular_file(master_file)) {
            return master_file.string();
        }

        // Rule #2: Filter and sort historical incremental numerical backups
        std::vector<fs::path> epoch_files;
        for (const auto& entry : fs::directory_iterator(directory)) {
            if (entry.is_regular_file()) {
                std::string filename = entry.path().filename().string();
                // 🎯 Extension Fix: Synchronized from .pt -> .ckpt to match main.cpp strings!
                if (filename.rfind("epoch_", 0) == 0 && entry.path().extension() == ".ckpt") {
                    epoch_files.push_back(entry.path());
                }
            }
        }

        if (epoch_files.empty()) return "";

        try {
            std::sort(epoch_files.begin(), epoch_files.end(), [](const fs::path& a, const fs::path& b) {
                std::string stem_a = a.stem().string();
                std::string stem_b = b.stem().string();
                int32_t num_a = std::stoi(stem_a.substr(6)); // Strip characters off "epoch_"
                int32_t num_b = std::stoi(stem_b.substr(6));
                return num_a < num_b;
            });
            return epoch_files.back().string();
        } catch (...) {
            auto latest = std::max_element(epoch_files.begin(), epoch_files.end(), [](const fs::path& a, const fs::path& b) {
